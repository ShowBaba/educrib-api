...
postRouter.route('/:postId/comments')
    ...
    .put(authenticate.varifyUser, (req, res, next) => {
        res.statusCode = 403;  // operation not supported
        res.end(`PUT operation not supported on /post/${req.params.postId}/comments`);
    }).delete(authenticate.varifyUser, authenticate.varifyAdmin, (req, res, next) => {
        Posts.findById(req.params.postId)
            .then((post) => {
                if (post != null) {
...
postRouter.route('/:postId/comments/:commentId')
    ...
    .post(authenticate.varifyUser, (req, res, next) => {
        ...
        Posts.findById(req.params.postId)
            .then((post) => {
                if ((req.user._id).equals(post.comments.id(req.params.commentId).author._id)) {
        ...
        Posts.findById(req.params.postId)
            .then((post) => {
                ...
                if ((req.user._id).equals(post.comments.id(req.params.commentId).author._id) || req.user._id.equals(post.author._id)) {
                    if (post != null && post.comments.id(req.params.commentId) != null) {
